...
of sigma/mu [%] are combined into one point. 
  ...
  vtx_syst     on the nominal row: sqrt(sum over the four shifted windows
               of (mean_variation - mean_nominal)^2 / 4)
  pooled       where no run reaches 300 events the single pooled fit is
...
ENERGY_COLUMNS = ("resistance", "energy", "energy_true", "variation",
                  "window", "n_run", "pooled", "n_events", "sigma_over_mu", "err_mean",
                  "scatter", "stat", "syst_tails", "drift", "chi2_ndf_drift", "vtx_syst",
                  "peak_mean", "runs")
RUN_COLUMNS = ("resistance", "energy", "energy_true", "run", "n_selected",
...
def weights_for(rows):
    return [1. / row["err_sigma_over_mu"] ** 2 if row["err_sigma_over_mu"] > 0 else 0. for row in rows]


def combine(point_rows, nominal_tails):
    ...
    pooled = 0
    if not nominal_rows and pooled_rows:
    ...
    weights = weights_for(nominal_rows)
    ...
    if fixed_runs and nominal_tails == "free":
        ...
        fixed_weights = weights_for([by_run[run] for run in fixed_runs])
...
def draw_runs(resistance, energy, run_rows, summary, pooled_row, outdir):
    ...
    for index, (label, y_values, y_errors, reference) in enumerate(panels):
        ...
        frame = common.run_axis_frame(labels, low - margin, high + margin,
                                      f"{resistance} #Omega, {energy} GeV, ;run;{label}")
        frame.Draw()
    ...
    common.save_canvas(canvas, os.path.join(outdir, "runs",
                                            f"peak_sigma_vs_run_{energy}GeV_{resistance}ohm.pdf"))
    common.save_canvas(canvas, os.path.join(outdir, "runs",
                                            f"peak_sigma_vs_run_{energy}GeV_{resistance}ohm.root"))


def draw_drift_check(resistance, energy_points, outdir):
    ...
    for index, (energy, summary, run_rows) in enumerate(energy_points):
        ...
        graph.Draw("P")
    canvas.cd()
    header = common.keep(ROOT.TLatex(0.01, 0.985, f"{resistance} #Omega, : per-run #sigma/#mu against a constant"))
    header.SetNDC()
    header.SetTextSize(0.018)
    header.Draw()
    common.save_canvas(canvas, os.path.join(outdir, f"drift_check_{resistance}ohm.png"))
    common.save_canvas(canvas, os.path.join(outdir, f"drift_check_{resistance}ohm.pdf"))
    common.save_canvas(canvas, os.path.join(outdir, f"drift_check_{resistance}ohm.root"))
...
def main():
    ...
    for (resistance, energy, variation), point_rows in sorted(grouped.items()):
        summary, run_rows = combine(point_rows, args.nominal_tails)
        if summary is None:
    ...
    for resistance in sorted({row["resistance"] for row in energy_rows}):
        ...
        if points:
            draw_drift_check(resistance, points, args.workdir)
