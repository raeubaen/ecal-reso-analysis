...
Reads 08_systematics.csv and draws, one column per
...
drift, and systematics), with markers for the points where the drift
...
09_resolution_terms.png. With --bes nominal the central value has the
...
COLUMNS = ("resistance", "energy", "energy_true", "window", "n_run", "pooled",
...
TERMS = {"bes", "sync", "drift", "stat", "vtx_syst", "bes_syst", "syn_syst"}
...
def draw_top(pad, rows, resistance, central="sigma_corr", bes_label="BES", error_column="err_total"):
    pad.SetGrid()
    pad.SetLeftMargin(0.13)
    raw = common.keep(styled(graph_of(rows, "sigma_raw"), 20, ROOT.kGray + 2))
    raw.SetTitle(f"{resistance} #Omega, E_{{true}} [GeV];#sigma/E  [%]")
    raw.Draw("APL")
    ...
    if corrected is not None:
        ...
        legend.AddEntry(corrected, f"- {bes_label} - synchrotron, err with systs", "pl")
...
def draw_terms(pad, rows, bes_column="bes"):
    pad.SetGrid()
    pad.SetLogy()
    pad.SetLeftMargin(0.13)
    ...
    frame.Draw()
    legend = common.keep(ROOT.TLegend(0.14, 0.12, 0.55, 0.12 + 0.032 * (len(TERMS) + 3)))
...
def main():
    ...
    points = [dict(resistance=row["resistance"], energy=row["energy"], energy_true=row["energy_true"],
                   window=row["window"], n_run=row["n_run"],
    ...
    for column, resistance in enumerate(resistances):
        ...
        draw_top(canvas.cd(column + 1), subset, resistance, central,
                 "BES (nominal)" if args.bes == "nominal" else "BES", error_column)
        draw_terms(canvas.cd(len(resistances) + column + 1), subset, bes_column)
    common.save_canvas(canvas, os.path.join(args.workdir, f"09_resolution_terms{suffix}.png"))
    common.save_canvas(canvas, os.path.join(args.workdir, f"09_resolution_terms{suffix}.pdf"))
    common.save_canvas(canvas, os.path.join(args.workdir, f"09_resolution_terms{suffix}.root"))
